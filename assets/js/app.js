        ...
        // Client-side chat limits (20 prompts per 10 hours)
        const DAILY_MAX = 20;              // total prompts allowed per 10 hours (per browser)
        const PERIOD_MS = 10 * 60 * 60 * 1000; // 10 hours
        const cooldownMs = 3000;           // 3 seconds between messages
        ...
        function enforcePromptLimitUI() {
            const overLimit = userPromptCount >= DAILY_MAX;
            if (overLimit) {
                setInputsEnabled(false);
                if (mainInput) mainInput.placeholder = '10-hour limit reached.';
                if (chatInput) chatInput.placeholder = '10-hour limit reached.';
                const now = Date.now();
                const remainingMs = Math.max(0, (quotaWindowStart + PERIOD_MS) - now);
                const hrs = Math.floor(remainingMs / 3600000);
                const mins = Math.floor((remainingMs % 3600000) / 60000);
                showError(`Limit reached (${DAILY_MAX} per 10h). Try again in ${hrs}h ${mins}m.`);
            }
        }
